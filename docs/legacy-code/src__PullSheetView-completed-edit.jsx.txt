...
export default function PullSheetView() {
  ...
  return (
    <main className="sc-page sc-pullsheet-detail-page">
      ...
      <HelpPanel>
        <p>Each card shows the finished product ordered by the customer on the left and the blank item the app plans to pull on the right. If the pairing is wrong, use Override Blank Pairing. Completed pull sheets can be unlocked for pairing corrections; those corrections do not move inventory automatically.</p>
      </HelpPanel>
      ...
      {closedJob ? (
        <SectionCard tone={editClosedMode ? 'warning' : 'default'} title={editClosedMode ? 'Completed editing is unlocked' : 'This pull sheet is completed or closed'}>
          <p>{editClosedMode
            ? 'You can change paired blank products on completed lines. Pairing corrections update the production record only; they do not reverse or deduct inventory.'
            : 'Click Edit Completed Pull Sheet to correct a paired blank product. Inventory corrections should be handled with Activity Feed undo or a manual inventory adjustment.'}</p>
        </SectionCard>
      ) : null}
          ...
          return (
            <article className="sc-pullsheet-line-card" key={key}>
              ...
              {closedLine && !editClosedMode ? (
                <div className="sc-warning-callout">This line is completed/closed. Use Edit Completed Pull Sheet to change the paired blank product.</div>
              ) : null}
