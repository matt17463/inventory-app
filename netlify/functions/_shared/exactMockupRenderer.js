...
// Exact mockups are ecommerce previews, not print-production files. Keep the
// original blank and artwork untouched in R2, but bound the rendered preview so
// large 300-DPI artwork cannot exhaust a Netlify function's memory allocation.
...
function maxDimension() {
  const configured = Number(process.env.MOCKUP_EXACT_MAX_DIMENSION || DEFAULT_MAX_DIMENSION);
  return Number.isFinite(configured) ? Math.min(Math.max(Math.round(configured), 1200), 3200) : DEFAULT_MAX_DIMENSION;
}
...
export async function renderExactMockup({ blankBytes, artworkBytes, placement, caption = null }) {
  const dimensionLimit = maxDimension();
  const blank = await sharp(blankBytes, { limitInputPixels: 120_000_000, sequentialRead: true })
    .rotate()
    .resize({ width: dimensionLimit, height: dimensionLimit, fit: 'inside', withoutEnlargement: true })
    .png({ compressionLevel: 6 })
    .toBuffer({ resolveWithObject: true });
  const baseWidth = blank.info.width;
  const baseHeight = blank.info.height;
  if (!baseWidth || !baseHeight) throw new Error('The blank image has invalid dimensions.');
  ...
  const artwork = await sharp(artworkBytes, { limitInputPixels: 120_000_000, sequentialRead: true })
    .rotate()
    .resize({
      width: Math.min(overlayWidth, dimensionLimit * 2),
      height: dimensionLimit * 2,
      fit: 'inside',
      withoutEnlargement: false,
    })
    .png({ compressionLevel: 6 })
    .toBuffer({ resolveWithObject: true });
  if (!artwork.info.width || !artwork.info.height) throw new Error('The artwork image has invalid dimensions.');
  const overlayHeight = artwork.info.height;
  const renderedOverlayWidth = artwork.info.width;
