...
Download `mockup-r2-storage-v0.8.5-patch.zip` into the Mac Downloads folder.

Run this complete block:

```bash
REPO_DIR="$HOME/inventory-app"
PATCH_ZIP="$HOME/Downloads/mockup-r2-storage-v0.8.5-patch.zip"
...
git fetch origin
git switch -c feature/mockup-r2-storage-v0.8.5
...
git status
git add -A
git commit -m "Move Mockup Studio image storage to Cloudflare R2 v0.8.5"
git push -u origin feature/mockup-r2-storage-v0.8.5
```

Open the pull request:

[Create the v0.8.5 GitHub pull request](https://github.com/matt17463/inventory-app/compare/main...feature/mockup-r2-storage-v0.8.5?expand=1)

Use this title:

```text
Move Mockup Studio image storage to Cloudflare R2 v0.8.5
```
...
3. Confirm production deployed version `0.8.5`.
...
If new R2 uploads must be temporarily stopped, keep v0.8.5 deployed and change:

```text
MOCKUP_STORAGE_PROVIDER=supabase
```

Then redeploy Netlify. Existing R2 images will remain readable because v0.8.5 supports both providers.
