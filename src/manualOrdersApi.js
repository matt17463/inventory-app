...
function generatedJobId(result) {
  const source = Array.isArray(result) ? result[0] : result;
  return source?.job_id
    || source?.generated_job_id
    || source?.generated?.job_id
    || source?.job?.id
    || null;
}

async function attachOutOfStockAssignment(result) {
  const jobId = generatedJobId(result);
  if (!jobId) return result;

  const assignment = await assignOutOfStockJobItemsToPendingStock(jobId);

  if (result && typeof result === 'object' && !Array.isArray(result)) {
    return {
      ...result,
      out_of_stock_assignment: assignment,
    };
  }
...
export async function createManualInvoiceOrder(order, items, generateJob = true) {
  ...
  if (!generateJob) return data;
  return attachOutOfStockAssignment(data);
}
