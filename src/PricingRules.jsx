...
export default function PricingRules() {
  ...
  async function saveRule() {
    ...
    else { setMessage(editingId ? 'Pricing rule updated.' : 'Pricing rule saved.'); setForm(empty); setEditingId(null); loadRules(); }
  }
  ...
  function edit(rule) {
    ...
    setForm({
      rule_name: rule.rule_name || '', decoration_type: rule.decoration_type || '', product_type: rule.product_type || '',
      base_price: rule.base_price ?? '', markup_percent: rule.markup_percent ?? '', flat_fee: rule.flat_fee ?? '',
      notes: rule.notes || '', is_active: rule.is_active !== false,
    });
  ...
  return <div className="sc-page-stack">
    ...
    <section className="sc-panel">
      <div className="sc-panel-header"><div><h3>{editingId ? 'Modify Pricing Rule' : 'Create Pricing Rule'}</h3></div></div>
      <div className="sc-form-grid">
        <label className="sc-field"><span>Rule Name</span><input value={form.rule_name} onChange={(e) => setForm({ ...form, rule_name: e.target.value })} /></label>
      ...
      <div className="sc-form-actions"><button className="sc-btn sc-btn-primary" onClick={saveRule}>{editingId ? 'Save Changes' : 'Save Rule'}</button>{editingId && <button className="sc-btn" onClick={() => { setEditingId(null); setForm(empty); }}>Cancel Edit</button>}</div>
    </section>
    <section className="sc-panel">
      <div className="sc-panel-header"><div><h3>Saved Rules</h3><p>Use Edit to modify a rule, or Delete to remove rules you no longer use.</p></div></div>
      <div className="sc-responsive-table-wrap"><table className="sc-table"><thead><tr><th>Name</th><th>Decoration</th><th>Product</th><th>Base</th><th>Markup</th><th>Flat Fee</th><th>Actions</th></tr></thead><tbody>{rules.map((rule) => <tr key={rule.id}><td>{rule.rule_name}</td><td>{rule.decoration_type || '—'}</td><td>{rule.product_type || '—'}</td><td>${Number(rule.base_price || 0).toFixed(2)}</td><td>{Number(rule.markup_percent || 0)}%</td><td>${Number(rule.flat_fee || 0).toFixed(2)}</td><td><button className="sc-btn sc-btn-small" onClick={() => edit(rule)}>Edit</button><button className="sc-btn sc-btn-danger sc-btn-small" onClick={() => deleteRule(rule.id)}>Delete</button></td></tr>)}{!rules.length && <tr><td colSpan="7" className="sc-empty-cell">No pricing rules saved yet.</td></tr>}</tbody></table></div>
    </section>
  </div>;
}
