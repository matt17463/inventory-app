...
export async function syncSupplierCatalogFeedIncremental(feedId, options = {}) {
  ...
  let offset = 0, complete = false, lastResponse = null, safetyCounter = 0;
  while (!complete) {
    safetyCounter += 1;
    ...
    onProgress({ offset, importedThisCall: Number(body.imported_this_call || 0), complete, message: body.message || '', raw: body });
